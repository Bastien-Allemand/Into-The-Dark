    ...
    private IEnumerator WaitAndLoadNextScene()
    {
        ...
        else if (currentChapterIndex < chaptersSequence.Count - 1)
        {
            ...
            if (chaptersSequence[currentChapterIndex].changeScene)
            {
                ...
                Debug.Log($"Nouveau Chapitre ! Changement de d�cor demand� : {activeSceneName}");
            }
            else
            {
                Debug.Log($"Nouveau Chapitre ! On conserve le d�cor : {activeSceneName}");
            }
        ...
        {
            Debug.Log("Jeu fini ! Fin de la s�quence compl�te.");
